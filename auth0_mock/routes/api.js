...
router.get( '/access_token', middleware.checkLogin, async ( req, res ) => {
    res.send( await Jwk_wrapper.createToken( access_token_claims( "", [`${helpers.auth0Url}/userinfo`] ) ) )
} );

// Returns id token for user
router.get( '/id_token', middleware.checkLogin, async ( req, res ) => {
    res.send( await Jwk_wrapper.createToken( helpers.removeNonceIfEmpty( id_token_claims() ) ) )
} );
...
router.post( '/oauth/token', middleware.checkLogin, async ( req, res ) => {
    ...
    const access_token_claim = access_token_claims( client_id, [`${helpers.auth0Url}/userinfo`] );
    console.log({access_token_claim});
    res.send( {
        "access_token": await Jwk_wrapper.createToken( access_token_claim ),
        "expires_in": 86400,
        "id_token": await Jwk_wrapper.createToken( helpers.removeNonceIfEmpty( id_token_claims( client_id ) ) ),
...
router.get( '/verify_token_test', middleware.checkLogin, async ( req, res ) => {
    await Jwk_wrapper.verify( await Jwk_wrapper.createToken( helpers.removeNonceIfEmpty( id_token_claims() ) ) );
