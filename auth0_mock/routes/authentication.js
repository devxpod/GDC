...
router.get('/authorize', async (req, res) => {
    ...
    if (prompt === 'none') {
        ...
        const access_token_claim = access_token_claims(audience, [audience,`${helpers.auth0Url}/userinfo`]);
        const vars = {
            "state": state,
            "code": "1234",
            "access_token": await Jwk_wrapper.createToken(access_token_claim),
            "expires_in": 86400,
            "id_token": await Jwk_wrapper.createToken(helpers.removeNonceIfEmpty(id_token_claims(audience))),
    ...
    return res.render('../templates/login_page', {redirect: redirect_uri, state});
});
...
router.post('/login', (req, res) => {
    ...
    return res.writeHead(302, {'Location': `${redirect}?code=1234&state=${encodeURIComponent(state)}`}).end();
});

// login route | alternative to using /authorizer->POST->/login flow
router.get('/login', (req, res) => {
    const logMsg = 'username = ' + req.query.username + ' && pw = ' + req.query.pw;
    ...
    return res.status(200).send(JSON.stringify(Auth.current_user) + '<br><br><b>you may continue with your auth0 needs</b>');
});
