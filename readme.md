...
```
**As of Localstack 4.0 `LOCALSTACK_API_KEY` is no longer supported.** Use `LOCALSTACK_AUTH_TOKEN` generated by the new token system from LS.
If you're using LocalStack Pro, create a file called `.env-gdc-local` and put your `LOCALSTACK_AUTH_TOKEN` in it.
```shell
